...
class Hero extends Phaser.GameObjects.Sprite {
  ...
  preUpdate(time, delta) {
    ...
    this.input.didPressJump = !this.isHeroDead() && Phaser.Input.Keyboard.JustDown(this.keys.up);
    
    if(!this.isHeroDead() && this.keys.left.isDown) {
      this.body.setAccelerationX(-1000);
      this.setFlipX(true);
      this.body.offset.x = 8;
    }
    else if(!this.isHeroDead() && this.keys.right.isDown) {
      this.body.setAccelerationX(1000);
      this.setFlipX(false);
